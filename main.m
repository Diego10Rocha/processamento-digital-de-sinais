...
load('sinais.mat');  % Carrega as variáveis x1_n e x2_n do arquivo sinais.mat
...
X2_k = fft(x2);  
f2 = (0:length(x2)-1)*(fs_x2/length(x2));  % Frequências para x2[n] em Hz

% Implementação manual do filtro passa-baixa com frequência de corte de 12 kHz (para D=4)
cutoff_x2 = 12000;  % Frequência de corte em Hz (ajustada para 24kHz)
...
% Fator de decimação para reduzir de 96 kHz para 24 kHz
D = 4;  % Fator de decimação (96 kHz / 24 kHz = 4)
...
fs_x2_new = fs_x2 / D;  % Nova taxa de amostragem: 24 kHz
...
X1_k = fft(x1);  
f1 = (0:length(x1)-1)*(fs_x1/length(x1));  % Frequências para x1[n] em Hz

% Fator de interpolação para aumentar de 8 kHz para 24 kHz
L = 3;  % Fator de interpolação (24 kHz / 8 kHz = 3)

% Substituição da função upsample por implementação manual
x1_n_upsampled = manual_upsample(x1, L);  % Inserir zeros (upsampling)

% Criar o filtro passa-baixa com frequência de corte de 4 kHz
fs_x1_new = fs_x1 * L;  % Nova taxa de amostragem: 24 kHz
...
soundsc(x_soma_normalized, fs_x1_new);  % Reproduz o sinal somado a 24 kHz
...
title('Domínio da Frequência de x2[n] Subamostrado (24 kHz) com Replicação');
xlabel('Frequência (Hz)');
...
title('Domínio da Frequência de x1[n] Superamostrado (24 kHz) com Replicação');
xlabel('Frequência (Hz)');
...
% audiowrite('soma_x1_x2_24kHz.wav', x_soma_normalized, fs_x1_new);